...
files.forEach(file => {
  ...
  text = text.replace(/<div className="p-4\s+font-mono text-sm">/g, '<div className="p-4 border border-black bg-gray-50 font-mono text-sm">');
  text = text.replace(/<div className="p-3\s+">/g, '<div className="p-3 border border-black bg-gray-50">');
  
  // Some extra fixes
  text = text.replace(/bg-\[#f8f8f8\]/g, 'bg-gray-50');
